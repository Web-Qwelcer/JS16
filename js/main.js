...
regForm.signRegButton.addEventListener("click", (event) => {
  ...
  else if (regEmail.value.length <= 6 || !(regEmail.value.trim().includes("@")) || !(regEmail.value.trim().includes("."))) {
    alert("Enter correct Email");
  }
  else if (regLogin.value == window.localStorage.getItem(`${regLogin.value}`)) {
...
loginButton.addEventListener("click", (event) => {
  let temp = JSON.parse(window.localStorage.getItem(`${loginForm.login.value}`));
  
  if (temp.password != loginForm.password.value || loginForm.login.value == "") {
    alert("Wrong DATA");
  }
  else {
...
regButton.addEventListener("click", () => {
  ...
  if (regName.value.trim().length == 0 && regEmail.value.trim().length == 0 && regLogin.value.trim().length == 0 && regPassword.value.trim().length == 0) {
    regFormDiv.classList.remove("active");
    regFormDiv.classList.add("hide");
    loginFormDiv.classList.remove("hide");
    loginFormDiv.classList.add("active");
  ...
  else if (regEmail.value.trim().length <= 6 || !regEmail.value.trim().includes("@") || !regEmail.value.trim().includes(".")) {
    alert("Enter correct Email");
  }
  else if (regLogin.value == window.localStorage.getItem(`${regLogin.value}`)) {
    alert(`Login ${regLogin.value} is exist`);
